    ...
    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-6">
      {skills.map((skill) => (
        <div key={skill.name} className="flex flex-col items-center group ml-4">
          <div className="text-4xl md:text-5xl p-4 bg-dracula-current/20 rounded-lg border border-border group-hover:border-dracula-cyan/45 transition-colors duration-200 mb-2">
            <div className={cn("transition-colors duration-200", skill.color)}>{skill.icon}</div>
          </div>

          <span className="text-[10px] font-bold uppercase text-center opacity-80 group-hover:opacity-100 group-hover:text-dracula-cyan transition-colors">
            {skill.name}
          </span>
        </div>
      ))}
  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar space-y-10">
        ...
        <section className="bg-dracula-current/10 p-6 rounded-lg border border-border">
